...
output_file = "/female_glioma_highestdose_fixed-MH-TACC.txt"
sink(paste0(saveto, output_file))
ma
sink()

# we save the forest plots
png(paste0(saveto, "/female_glioma_highestdose.png"), 11,7, units = "in", res = 300)
forest(ma)
...
pdf(paste0(saveto, "/female_glioma_highestdose.pdf"), 11,7)
forest(ma)
grid.text( "Meta-analysis of gliomas in female rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
...
output_file = "/female_glioma_doseaggr_fixed-MH-TACC.txt"
sink(paste0(saveto, output_file))
ma_aggr
sink()

# we save the forest plots
png(paste0(saveto, "/female_glioma_dosesaggr.png"), 11,7.5, units = "in", res = 300)
forest(ma_aggr)
...
pdf(paste0(saveto, "/female_glioma_dosesaggr.pdf"), 11,7.5)
forest(ma_aggr)
grid.text( "Meta-analysis of gliomas in female rats,\nnumbers from different exposure levels combined", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
