...
png(paste0(saveto, "/females_schwannoma_alldoses_frTACC.png"), 11,4, units = "in", res = 300)
forest(ma)
...
pdf(paste0(saveto, "/females_schwannoma_alldoses_frTACC.pdf"), 11,4)
forest(ma)
grid.text( "Meta-analysis of cardiac schwannomas in female rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, fractional per study treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
...
output_file <- "/females_schwannoma_frTACC_lin_ModelSummary.txt"
...
#pdf plot
pdf(paste0(saveto, "/females_schwannoma_frTACC_lin.pdf"), 7,4)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y", col="blue", ylim=c(0.3, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="schwannoma odds ratio", xlab="SAR, W/kg", axes=FALSE)
...
#png plot
png(paste0(saveto, "/females_schwannoma_frTACC_lin.png"), 7,4, unit = "in", res= 300)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y", col="blue", ylim=c(0.3, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="schwannoma odds ratio", xlab="SAR, W/kg", axes=FALSE)
...
output_file <- "/females_schwannoma_frTACC_quad_ModelSummary.txt"
...
# plot pdf
pdf(paste0(saveto, "/females_schwannoma_frTACC_quad.pdf"), 7,4)

with(predict(quad_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, log="y",  type="l", col="blue", ylim=c(0.1, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="schwannoma odds ratio", xlab="SAR, W/kg", axes=FALSE)
...
png(paste0(saveto, "/females_schwannoma_frTACC_quad.png"), 7,4, unit= "in", res = 300)
