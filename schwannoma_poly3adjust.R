...
###### GSM rats, males, heart schwannomas ######
...
extra_columns <- c("System", "Organ", "Site", "Number Examined", "Lesion Name", "Lesion Qualifier")
...
# Create a new variable "tumor" with conditions (Malignant Heart Schwannoma, not metastasis from another organ)
filtered_df <- filtered_df %>%
  mutate(tumor = ifelse(Organ == "Heart" &  !grepl("Metastatic", `Lesion Qualifier`) & grepl("Schwannoma", `Lesion Name`), 1, 0))
...
###### CDMA rats, males, heart schwannomas ######
...
output_file <- "res/all-male_heartschwannoma_poly3add1_formeta.txt"
...
print("poly3test: Dunnett & ADD1 in GSM+CDMA male rats - heart schwannoma incidence")
...
#### !!!! ---> Use these numbers to fill data for the Excel table: data_male_schwannoma_poly3adj_alldoses.xlsx
...
###### CDMA rats, females, brain schwannomas ######
...
output_file <- "res/all-female_schwannoma_poly3add1_formeta.txt"
...
# Any command output following this will be written to the file
# Example function calls
print("poly3test: Dunnett & ADD1 in GSM+CDMA female rats - schwannoma incidence")
...
### !!!! ---> Use these numbers to fill data for the Excel table: data_female_schwannoma_poly3adj_alldoses.xlsx
...
output_file <- "res/all-m+f_schwannoma_poly3add1_formeta.txt"
...
# Any command output following this will be written to the file
# Example function calls
print("poly3test: Dunnett & ADD1 in GSM+CDMA male+female rats - schwannoma incidence")
...
print("poly3test: Williams-Type & ADD1 in GSM+CDMA male+female rats - schwannoma incidence - test for trend")
...
nadj_table <- data.frame("0" = nadj[1],
                         ...
                         "6" = nadj[4])
colnames(nadj_table) <- c("0 W/kg", "1.5 W/kg", "3 W/kg", "6 W/kg")
# Display the table of poly-3 adjusted animal numbers per exposure group
print(nadj_table)

### !!!! ---> Use these numbers to fill data for the Excel table: 
# data_m+f_schwannoma_poly3adj_alldoses.xlsx
