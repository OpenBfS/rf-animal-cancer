...
# check if added numbers are equal to 4x0.5x2=4
...
# modify Adey 1999 study (+0.5 cases to sham and exposed)
TACC = 0.5
dat$exposed_N_cases[10]  = dat$exposed_N_cases[10] +TACC
dat$exposed_N_all[10]  = dat$exposed_N_all[10]  + 2*TACC

dat$sham_N_cases[10]  = dat$sham_N_cases[10]  + TACC
dat$sham_N_all[10]  = dat$sham_N_all[10]  + 2*TACC
...
png(paste0(saveto, "/m+f_glioma_alldoses_frTACC.png"), 11,6, units = "in", res = 300)
forest(ma)
...
pdf(paste0(saveto, "/m+f_glioma_alldoses_frTACC.pdf"), 11,6)
forest(ma)
grid.text( "Meta-analysis of gliomas in m+f rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, fractional per study treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
...
ORs = as.numeric(forest$effect.format[4:16])
ORs
...
doses <- c(1.5, 3, 6, 0.001, 0.03, 0.1, 0.16, 1.6, 1.3,1.3, 1.1, 1.0, 1.0)
...
Adey1999_reference_row <- data.frame(author = "", id = "", type = "",
                                dose = 0, cases = events_control[10], n = total_control[10],
                                logrr = 0, se = NA, ci_lower= NA, ci_upper = NA, stringsAsFactors = FALSE)

Adey2000_reference_row <- data.frame(author = "", id = "", type = "",
                                      dose = 0, cases = events_control[11], n = total_control[11],
                                      logrr = 0, se = NA, ci_lower= NA, ci_upper = NA, stringsAsFactors = FALSE)

ZookCW_reference_row <- data.frame(author = "", id = "", type = "",
                                     dose = 0, cases = events_control[12], n = total_control[12],
                                     logrr = 0, se = NA, ci_lower= NA, ci_upper = NA, stringsAsFactors = FALSE)

ZookP_reference_row <- data.frame(author = "", id = "", type = "",
                                     dose = 0, cases = events_control[13], n = total_control[13],
                                     logrr = 0, se = NA, ci_lower= NA, ci_upper = NA, stringsAsFactors = FALSE)
...
data_frame <- rbind(data_frame[1:13,], Adey1999_reference_row, data_frame[-(1:13),])
data_frame <- rbind(data_frame[1:15,], Adey2000_reference_row, data_frame[-(1:15),])
data_frame <- rbind(data_frame[1:17,], ZookCW_reference_row, data_frame[-(1:17),])
data_frame <- rbind(data_frame[1:19,], ZookP_reference_row, data_frame[-(1:19),])

data_frame$author <- rep(c("NTP[2018][rats][GSM+CDMA]", "Ramazzini[2018]", "Anderson[2004]", "LaRegina[2003]", "Adey[1999]", "Adey[2000]","Zook[2001][CW]","Zook[2001][P]" ), c(4,4,3,2,2,2,2,2))
data_frame$id <- rep(c(1, 2, 3, 4, 5, 6, 7, 8), c(4,4,3,2,2,2,2,2))
...
write_xlsx(data_frame, path  = paste0(saveto, "/m+f_glioma_frTACC_dosres.xlsx"))
...
saveto <- "./dosresmeta"
data_bin <- read_excel(paste0(saveto, "/m+f_glioma_frTACC_dosres.xlsx"))
...
output_file <- "/m+f_glioma_frTACC_lin_freemov_ModelSummary.txt"
...
#pdf plot
pdf(paste0(saveto, "/m+f_glioma_frTACC_lin_freemov.pdf"), 7,4)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l",log="y",col="blue", ylim=c(0.8, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="glioma odds ratio", xlab="SAR, W/kg", axes=FALSE)
...
lin_bin <- dosresmeta(formula=logrr ~ dose, type=type, id=id,
                      lb=ci_lower, ub=ci_upper, 
                      #se=se, 
                      cases=cases, n=n, covariance = "gl",  method="fixed", proc="1stage", data=data_bin[9:21,] ) 

output_file <- "/m+f_glioma_frTACC_lin_Restr_ModelSummary.txt"
...
#pdf plot
pdf(paste0(saveto, "/m+f_glioma_frTACC_lin_Restr.pdf"), 7,4)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", col="blue", log="y", ylim=c(0.05, (max(exp(data_bin[9:21,]$logrr))+0.5*(max(exp(data_bin[9:21,]$logrr))))), ylab="glioma odds ratio", xlab="SAR, W/kg", axes=FALSE)
  
  lines(dose, ci.lb, lty=2)
  
  lines(dose, ci.ub, lty=2)})
points(data_bin[9:21,]$dose, exp(data_bin[9:21,]$logrr))
...
#png plot
png(paste0(saveto, "/m+f_glioma_frTACC_lin_Restr.png"), 7,4, unit = "in", res= 300)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", col="blue", log="y", ylim=c(0.05, (max(exp(data_bin[9:21,]$logrr))+0.5*(max(exp(data_bin[9:21,]$logrr))))), ylab="glioma odds ratio", xlab="SAR, W/kg", axes=FALSE)
...
output_file <- "/m+f_glioma_frTACC_quad_freemov_ModelSummary.txt"
...
#pdf plot
pdf(paste0(saveto, "/m+f_glioma_frTACC_quad_freemov.pdf"), 7,4)
with(predict(quad_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y",col="blue", ylim=c(0.7, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="glioma odds ratio", xlab="SAR, W/kg", axes=FALSE)
...
#png plot
png(paste0(saveto, "/m+f_glioma_frTACC_quad_freemov.png"), 7,4, unit = "in", res= 300)
with(predict(quad_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y",col="blue", ylim=c(0.7, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="glioma odds ratio", xlab="SAR, W/kg", axes=FALSE)
...
output_file <- "/m+f_glioma_frTACC_quad_Restr_ModelSummary.txt"
...
#pdf plot
pdf(paste0(saveto, "/m+f_glioma_frTACC_quad_Restr.pdf"), 7,4)
with(predict(quad_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y",col="blue", ylim=c(0.05, (max(exp(data_bin[9:21,]$logrr))+2*(max(exp(data_bin[9:21,]$logrr))))), ylab="glioma odds ratio", xlab="SAR, W/kg", axes=FALSE)
...
#png plot
png(paste0(saveto, "/m+f_glioma_frTACC_quad_Restr.png"), 7,4, unit = "in", res= 300)
with(predict(quad_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y",col="blue", ylim=c(0.05, (max(exp(data_bin[9:21,]$logrr))+2*(max(exp(data_bin[9:21,]$logrr))))), ylab="glioma odds ratio", xlab="SAR, W/kg", axes=FALSE)
