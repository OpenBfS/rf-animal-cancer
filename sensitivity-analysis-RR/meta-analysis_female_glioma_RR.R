...
dat$design = c("free, >18h/day","free, >18h/day", "restrained, <= 6h/day", "restrained, <= 6h/day")



##### Step 2: Meta-analysis and forest plot for female for highest doses #####
...
ma <<- metabin(event.e = exposed_N_cases,
                            ...
                            method = "MH",
                            incr = "TACC",  # otherwise we'd get an RR of 2.0 for the Adey1999 study
...
output_file = "/female_glioma_highestdose_fixed-MH-TACC_RR.txt"
sink(paste0(saveto, output_file))
ma
sink()

# we save the forest plots
png(paste0(saveto, "/female_glioma_highestdose_RR.png"), 11,7, units = "in", res = 300)
forest(ma)
...
pdf(paste0(saveto, "/female_glioma_highestdose_RR.pdf"), 11,7)
forest(ma)
grid.text( "Meta-analysis of gliomas in female rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
...
##### Step 3: Descriptive forest plot for female for aggregated dose groups#####
# load the according Excel file

dat_aggr  <- read_excel("data_female_glioma_poly3adj_dose-aggreg.xlsx")

# create a variable describing the exposure design of the studies, for subgroup analysis
dat_aggr$design = c("free, >18h/day","free, >18h/day", "restrained, <= 6h/day", "restrained, <= 6h/day")
...
ma_aggr <<- metabin(event.e = exposed_N_cases,
                                ...
                                #ethod.tau = "DL",            #use DerSimonian-Laird estimator to calculate between-study variance tau
                                #method.tau.ci = "J",          #use Method by Jackson (2013) to calculate confidence interval for tau^2 and tau
...
output_file = "/female_glioma_doseaggr_fixed-MH-TACC_RR.txt"
sink(paste0(saveto, output_file))
ma_aggr
sink()

# we save the forest plots
png(paste0(saveto, "/female_glioma_dosesaggr_RR.png"), 11,7.5, units = "in", res = 300)
forest(ma_aggr)
grid.text( "Meta-analysis of gliomas in female rats,\nnumbers from different exposure levels combined", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()

pdf(paste0(saveto, "/female_glioma_dosesaggr_RR.pdf"), 11,7.5)
forest(ma_aggr)
grid.text( "Meta-analysis of gliomas in female rats,\nnumbers from different exposure levels combined", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
