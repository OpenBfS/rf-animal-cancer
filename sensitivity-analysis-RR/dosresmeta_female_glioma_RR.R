...
dat  <- read_excel("data_female_glioma_poly3adj_alldoses.xlsx")
...
total_N_Anderson <- sum(dat$exposed_N_all[7:8]) + sum(dat$sham_N_all[7])

total_N_LaRegina <- sum(dat$exposed_N_all[9]) + sum(dat$sham_N_all[9])
...
group_sizes_Anderson <- c(dat$sham_N_all[7], dat$exposed_N_all[7:8])

group_sizes_LaRegina <- c(dat$sham_N_all[9], dat$exposed_N_all[9])
...
# 3(arms) x 0.5 )= 1.5 total cases for Anderson, 1 case for LaRegina
cases_to_distribute = 1.5
frac_CC_Anderson <- cases_to_distribute * (group_sizes_Anderson / total_N_Anderson)

cases_to_distribute = 1
frac_CC_LaRegina <- cases_to_distribute * (group_sizes_LaRegina / total_N_LaRegina)

# round to three decimals
frac_CC_NTP <- round(frac_CC_NTP, 3)
frac_CC_Anderson <- round(frac_CC_Anderson, 3)
frac_CC_LaRegina <- round(frac_CC_LaRegina, 3)

# adjust vectors to match data frame structure with the exposed and sham columns. Round to two decimals
exposed_add <- round ( c(frac_CC_NTP[2:4], frac_CC_Anderson[2:3], frac_CC_LaRegina[2]
                         ),2)
sham_add <- round ( c(rep(frac_CC_NTP[1], 3), rep(frac_CC_Anderson[1], 2), frac_CC_LaRegina[1]
                      ),2)

# check 
sum(2*exposed_add) + 2* sum(sham_add[c(1,4,6
                                       )])

# modify data frame - total cases are going to get double (cases get added to all four cells of a 2x2 table)
vec_to_change = c(1,2,3,7,8,9)
dat$exposed_N_cases[vec_to_change]  = dat$exposed_N_cases[vec_to_change] + exposed_add
dat$exposed_N_all[vec_to_change]  = dat$exposed_N_all[vec_to_change]  + 2*exposed_add

dat$sham_N_cases[vec_to_change]  = dat$sham_N_cases[vec_to_change]  + sham_add
dat$sham_N_all[vec_to_change]  = dat$sham_N_all[vec_to_change]  + 2*sham_add


##### Step 2: Descriptive forest plot for female #####
...
ma <<- metabin(event.e = exposed_N_cases,
               ...
               title = "Meta-Analysis of gliomas in female rats",
...
png(paste0(saveto, "/female_glioma_alldoses_frTACC_RR.png"), 11,6, units = "in", res = 300)
forest(ma)
grid.text( "Meta-analysis of gliomas in female rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
...
pdf(paste0(saveto, "/female_glioma_alldoses_frTACC_RR.pdf"), 11,6)
forest(ma)
grid.text( "Meta-analysis of gliomas in female rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, fractional per study treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
...
RRs = as.numeric(forest$effect.format[4:12])
RRs

# extract confidence intervals
CI = forest$ci.format[4:12]
...
doses <- c(1.5, 3, 6, 0.001, 0.03, 0.1, 0.16, 1.6, 1.3)
...
data_frame$author <- rep(c("NTP[2018][rats][GSM+CDMA]", "Ramazzini[2018]", "Anderson[2004]", "LaRegina[2003]"), c(4,4,3,2))
data_frame$id <- rep(c(1, 2, 3, 4), c(4,4,3,2))
...
write_xlsx(data_frame, path  = paste0(saveto, "/female_glioma_frTACC_dosres.xlsx"))
...
saveto <- "./dosresmeta"
data_bin <- read_excel(paste0(saveto, "/female_glioma_frTACC_dosres.xlsx"))
...
lin_bin <- dosresmeta(formula=logrr ~ dose, type=type, id=id,
                      ...
                      #se=se, 
                      cases=cases, n=n, covariance = "gl",  method="fixed", proc="1stage", data=data_bin[1:8,] ) 

output_file <- "/female_glioma_frTACC_lin_freemov_ModelSummary_RR.txt"
...
#pdf plot
pdf(paste0(saveto, "/female_glioma_frTACC_lin_freemov_RR.pdf"), 7,4)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y", col="blue", ylim=c(0.5, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="glioma risk ratio", xlab="SAR, W/kg", axes=FALSE)
  
  lines(dose, ci.lb, lty=2)
  
  lines(dose, ci.ub, lty=2)})
points(data_bin[1:8,]$dose, exp(data_bin[1:8,]$logrr))

axis(2)  # y-axis default (log scale)
axis(1, at=xticks_lnSAR, labels=xticks_SAR)  # x-axis with SAR values
box()

grid.text( "Dose-response meta-analysis (linear) \n of gliomas in female rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.3))
grid.text( paste0("p-value = ", round(obj$Wald.test[3], 4), "\nAIC = ", round(obj$AIC, 4)), x = 0.5, y = 0.7, gp = gpar(cex = 1))

dev.off()

#png lot
png(paste0(saveto, "/female_glioma_frTACC_lin_freemov_RR.png"), 7,4, unit = "in", res= 300)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y", col="blue", ylim=c(0.5, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="glioma risk ratio", xlab="SAR, W/kg", axes=FALSE)
...
###### subgroup analysis - only restraint / brain-averaged SAR studies ######
...
output_file <- "/female_glioma_frTACC_lin_Restr_ModelSummary_RR.txt"
...
#pdf plot
pdf(paste0(saveto, "/female_glioma_frTACC_lin_Restr_RR.pdf"), 7,4)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y", col="blue", ylim=c(0.3, (max(exp(data_bin[9:13,]$logrr))+0.1*(max(exp(data_bin[9:13,]$logrr))))), ylab="glioma risk ratio", xlab="SAR, W/kg", axes=FALSE)
...
#plot png
png(paste0(saveto, "/female_glioma_frTACC_lin_Restr_RR.png"), 7,4, unit = "in", res= 300)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y", col="blue", ylim=c(0.3, (max(exp(data_bin[9:13,]$logrr))+0.1*(max(exp(data_bin[9:13,]$logrr))))), ylab="glioma risk ratio", xlab="SAR, W/kg", axes=FALSE)
...
output_file <- "/female_glioma_frTACC_quad_freemov_ModelSummary_RR.txt"
...
#pdf plot
pdf(paste0(saveto, "/female_glioma_frTACC_quad_freemov_RR.pdf"), 7,4)
with(predict(quad_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y",col="blue", ylim=c(0.3, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="glioma risk ratio", xlab="SAR, W/kg", axes=FALSE)
...
#png plot
png(paste0(saveto, "/female_glioma_frTACC_quad_freemov_RR.png"), 7,4, unit = "in", res= 300)
...
output_file <- "/female_glioma_frTACC_quad_Restr_ModelSummary_RR.txt"
...
#üdf plot
pdf(paste0(saveto, "/female_glioma_frTACC_quad_Restr_RR.pdf"), 7,4)
with(predict(quad_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y", col="blue", ylim=c(0.1, (max(exp(data_bin[9:13,]$logrr))+0.1*(max(exp(data_bin[9:13,]$logrr))))), ylab="glioma risk ratio", xlab="SAR, W/kg", axes=FALSE)
...
#png plot
png(paste0(saveto, "/female_glioma_frTACC_quad_Restr_RR.png"), 7,4, unit = "in", res= 300)
