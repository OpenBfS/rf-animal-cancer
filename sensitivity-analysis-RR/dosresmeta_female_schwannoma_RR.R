...
# total_N_Falcioni <- sum(dat$exposed_N_all[4:6]) + sum(dat$sham_N_all[4]) # not necessary for Falcioni since all exposure arms have cases
...
exposed_add <- round ( c(frac_CC_NTP[2:4] #, frac_CC_Falcioni[2:4]
                         ), 2)
sham_add <- round ( c(rep(frac_CC_NTP[1], 3) #, rep(frac_CC_Falcioni[1], 3)
                      ), 2)

# check, should result in 2x2 (last x2 because of the same number of animals added to non-events)
...
##### Step 2: Descriptive forest plot for females #####
...
ma <<- metabin(event.e = exposed_N_cases,
               ...
               title = "Meta-Analysis of cardiac schwannomas in female rats, all doses",
...
png(paste0(saveto, "/females_schwannoma_alldoses_frTACC_RR.png"), 11,4, units = "in", res = 300)
forest(ma)
...
pdf(paste0(saveto, "/females_schwannoma_alldoses_frTACC_RR.pdf"), 11,4)
forest(ma)
grid.text( "Meta-analysis of cardiac schwannomas in female rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, fractional per study treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
...
RRs = as.numeric(forest$effect.format[4:9])
RRs
...
write_xlsx(data_frame, path  = paste0(saveto, "/female_schwannoma_frTACC_dosres.xlsx"))
...
saveto <- "./dosresmeta"
data_bin <- read_excel(paste0(saveto, "/female_schwannoma_frTACC_dosres.xlsx"))
...
output_file <- "/females_schwannoma_frTACC_lin_ModelSummary_RR.txt"
...
#pdf plot
pdf(paste0(saveto, "/females_schwannoma_frTACC_lin_RR.pdf"), 7,4)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y", col="blue", ylim=c(0.3, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="schwannoma risk ratio", xlab="SAR, W/kg", axes=FALSE)
...
grid.text( "Dose-response meta-analysis (linear) \n of cardiac schwannomas in female rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.3))
grid.text( paste0("p-value = ", round(obj$Wald.test[3], 4), "\nAIC = ", round(obj$AIC, 4)), x = 0.5, y = 0.7, gp = gpar(cex = 1))

dev.off()

#png plot
png(paste0(saveto, "/females_schwannoma_frTACC_lin_RR.png"), 7,4, unit = "in", res= 300)
with(predict(lin_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, type="l", log="y", col="blue", ylim=c(0.3, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="schwannoma risk ratio", xlab="SAR, W/kg", axes=FALSE)
...
output_file <- "/females_schwannoma_frTACC_quad_ModelSummary_RR.txt"
...
# plot pdf
pdf(paste0(saveto, "/females_schwannoma_frTACC_quad_RR.pdf"), 7,4)

with(predict(quad_bin, dosex_bin, order=TRUE, exp=TRUE), {plot(dose, pred, log="y",  type="l", col="blue", ylim=c(0.1, (max(exp(data_bin$logrr))+0.1*(max(exp(data_bin$logrr))))), ylab="schwannoma risk ratio", xlab="SAR, W/kg", axes=FALSE)
...
png(paste0(saveto, "/females_schwannoma_frTACC_quad_RR.png"), 7,4, unit= "in", res = 300)
