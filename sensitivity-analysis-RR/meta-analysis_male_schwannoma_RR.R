...
dat <-  dat[c(3,6,7),] # Chou 1992 included
...
ma <<- metabin(event.e = exposed_N_cases,
                            ...
                            title = "Meta-Analysis of cardiac schwannomas in male rats",
                            
                            random = FALSE,                #no random effects modeled
                            common = TRUE,                #default fixed effect model
                            sm = "RR",                   #use RR as summary measure 
                            incr = "TACC",
                            allstudies = TRUE, # force calculation for double-zero studies
                            method = "MH",
...
output_file = "/males_schwannoma_highestdose_fixed-MH-TACC_RR.txt"
sink(paste0(saveto, output_file))
ma
sink()

# we save the forest plots
png(paste0(saveto, "/males_schwannoma_highestdose_RR.png"), 11,4, units = "in", res = 300)
forest(ma)
...
pdf(paste0(saveto, "/males_schwannoma_highestdose_RR.pdf"), 11,4)
forest(ma)
grid.text( "Meta-analysis of cardiac schwannomas in male rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
...
dat <-  dat[-c(3),] # Chou 1992 included
...
# we save the model output to text
output_file = "/males_schwannoma_highestdose_fixed-MH-TACC_noDblZero_RR.txt"
sink(paste0(saveto, output_file))
ma_sens
sink()

# we save the forest plots
png(paste0(saveto, "/males_schwannoma_highestdose_noDblZero_RR.png"), 11,4, units = "in", res = 300)
forest(ma_sens)
grid.text( "Meta-analysis of cardiac schwannomas in male rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()

pdf(paste0(saveto, "/males_schwannoma_highestdose_noDblZero_RR.pdf"), 11,4)
forest(ma_sens)
grid.text( "Meta-analysis of cardiac schwannomas in male rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
...
dat_aggr  <- read_excel("data_male_schwannoma_poly3adj_dose-aggreg.xlsx")
...
# we save the model output to text
output_file = "/males_schwannoma_doseaggr_fixed-MH-TACC_RR.txt"
sink(paste0(saveto, output_file))
ma_aggr
sink()

# we save the forest plots
png(paste0(saveto, "/males_schwannoma_dosesaggr_RR.png"), 11,4, units = "in", res = 300)
forest(ma_aggr)
grid.text( "Meta-analysis of cardiac schwannomas in male rats,\nnumbers from three exposure levels combined", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()

pdf(paste0(saveto, "/males_schwannoma_dosesaggr_RR.pdf"), 11,4)
forest(ma_aggr)
grid.text( "Meta-analysis of cardiac schwannomas in male rats,\nnumbers from three exposure levels combined", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
