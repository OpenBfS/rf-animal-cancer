...
output_file = "/females_schwannoma_highestdose_fixed-MH-TACC.txt"
sink(paste0(saveto, output_file))
ma
sink()

# we save the forest plots
png(paste0(saveto, "/females_schwannoma_highestdose.png"), 11,4, units = "in", res = 300)
forest(ma)
...
pdf(paste0(saveto, "/females_schwannoma_highestdose.pdf"), 11,4)
forest(ma)
grid.text( "Meta-analysis of cardiac schwannomas in female rats", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
...
# we save the model output to text
output_file = "/females_schwannoma_doseaggr_fixed-MH-TACC.txt"
sink(paste0(saveto, output_file))
ma_aggr
sink()

# we save the forest plots
png(paste0(saveto, "/females_schwannoma_dosesaggr.png"), 11,4, units = "in", res = 300)
forest(ma_aggr)
...
pdf(paste0(saveto, "/females_schwannoma_dosesaggr.pdf"), 11,4)
forest(ma_aggr)
grid.text( "Meta-analysis of cardiac schwannomas in female rats,\nnumbers from three exposure levels combined", x = 0.5, y = 0.9, gp = gpar(cex = 1.5))
grid.text( "(NTP numbers are poly3-adjusted, treatment arm continuity correction)", x = 0.5, y = 0.1, gp = gpar(cex = 1))
dev.off()
